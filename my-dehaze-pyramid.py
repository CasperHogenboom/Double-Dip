...
def dehaze(image_name: str, image: np.array, num_iter: int=8000,
           plot_during_training: bool=True, show_every: int=500):
    dehazer = Dehazer(image_name, image, num_iter, plot_during_training, show_every)
    dehazer.optimize()
    dehazer.finalize()
...
class Dehazer(object):
    def __init__(self, image_name, image, num_iter=8000, plot_during_training=True, show_every=500):
        ...
        #self.mse_loss = torch.nn.MSELoss().type(torch.FloatTensor)
        self.blur_loss = StdLoss().type(torch.cuda.FloatTensor)
        #self.blur_loss = StdLoss().type(torch.FloatTensor)
    ...
    def _init_nets(self, pyramid=True):
    ...
    def _init_inputs(self, pyramid=True):
        ...
        if pyramid:
            mask_noises = create_augmentations(torch_to_np(get_noise(3, 'noise',
                                                                (self.images[0].shape[1], self.images[0].shape[2]), var=1/10.)
                                                                .type(torch.cuda.FloatTensor).detach()))
        else:
            mask_noises = create_augmentations(torch_to_np(get_noise(self.input_depth, 'noise',
                                                                        (self.images[0].shape[1], self.images[0].shape[2]),
                                                                        var=1 / 10.).type(torch.cuda.FloatTensor).detach()))
        ...
        # Dense net requires only 3 channels input instead of 8
        self.mask_net_inputs = [np_to_torch(mask_noise).type(torch.cuda.FloatTensor).detach()
                                for mask_noise in mask_noises]
    ...
    def _obtain_current_result(self):
        ...
        # TODO: changed to loss, but better use SSIM and compare with ground truth (still unsupervised then?)
...
if __name__ == "__main__":
    ...
    #image = prepare_image("/home/jupyter/DIP-shit/images/hongkong.png")
    image = prepare_image("/home/jupyter/DIP-shit/dataset/hazy/01_hazy.png")
    # dehaze(os.path.basename(sys.argv[1]), image)
    dehaze("hazy01.png", image)
